...
double centerPulseWidth = 1.35;
double pulseVariance = .9;
...
void init_sonar_timer() {
	...
	TIMER3_TBMR_R |=(TIMER_TBMR_TBCDIR | TIMER_TBMR_TBCMR | TIMER_TBMR_TBMR_CAP); //pg 729
...
/*
 * Performs 180 scans using both the IR and sonar sensors.
 * IR data is the average of three readings. Data returned is in the order of right to left.
 */
struct distance_info * perform_scan() {
	...
	while (1) {
		...
		distances[arrayPos].sonar_cm = pingDistance;
...
/*
 * Reads distance data and returns an array of detected objects.
 * Ignores distances > 80 cm and objects < 1 cm in diameter due to sensor unreliability.
 */
	...
	for(i = 0;i < 180;i++)
	{
		if(inObject == 1)
		{
			if(abs(distances[i].IR_cm - distances[i - 1].IR_cm) > CHAOS)//not in same object
			{
				...
				double size = pow(pow(distances[startPosition].sonar_cm, 2.0) + pow(distances[i - 1].sonar_cm, 2.0) - 2.0 * distances[startPosition].sonar_cm * distances[i - 1].sonar_cm * cosine, .5);
				if(size > 1)
				{
					scan_angles[distance_counter] = (i + startPosition - 1) / 2;
					scan_distance[distance_counter] = (i + startPosition - 1) / 2, distances[(i + startPosition - 1) / 2].sonar_cm + (size / 2.0);
					distance_counter++;
					char message[100];
					sprintf(message, "%d\t%fcm.\t%fcm.\n\r", (i + startPosition - 1) / 2, distances[(i + startPosition - 1) / 2].sonar_cm + (size / 2.0), size);
...
// counter used for GUI
int get_distanceCounter()
{
